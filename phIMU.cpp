...
void IMU::configHW(bool _ADO, uint8_t _intPin, uint8_t _adoPin){
	ADO=_ADO;
	intPin=_intPin;
	adoPin=_adoPin;
};

void IMU::configIMU(uint8_t _Ascale,uint8_t _Gscale, uint8_t _Mscale,uint8_t _Mmode){
	uint8_t Gscale = _Gscale;
	uint8_t Ascale = _Ascale;
	uint8_t Mscale = _Mscale; // Choose either 14-bit or 16-bit magnetometer resolution
	uint8_t Mmode = _Mmode;        // 2 for 8 Hz, 6 for 100 Hz continuous magnetometer data read
};
...
bool IMU::startUp(){
	...
	Serial.println("Startin IMU.....");
	Wire.begin(I2C_MASTER, 0x00, I2C_PINS_18_19, I2C_PULLUP_EXT, I2C_RATE_100);
	
	//Set interrupt and ado pin modes
	pinMode(intPin, INPUT);
  	digitalWrite(intPin, LOW);
  	pinMode(adoPin, OUTPUT);
  	digitalWrite(adoPin, HIGH);

  	//Verify communication with MPU9250
  	byte c = readByte(MPU9250_ADDRESS, WHO_AM_I_MPU9250);  // Read WHO_AM_I register for MPU-9250
  	if (c == 0x71) {
  		Serial.println("IMU online and running");
  		postStartActions();	// WHO_AM_I should always be 0x71  
		...
		byte d = readByte(AK8963_ADDRESS, AK8963_WHO_AM_I);  // Read WHO_AM_I register for AK8963
  		...
  		Serial.println("IMU communication failure");
  		return false;
  	}
};

void IMU::postStartActions(){
	if(selfTestOnStart){
		
		selfTestMPU9250(SelfTest);
		Serial.println("Self test complete");
	}
	if(calibrateOnStart){
		Serial.println("Running MPU9250 calibration...");
...
void IMU::update(){
	...
	yaw   = atan2(2.0f * (qM[1] * qM[2] + qM[0] * qM[3]), qM[0] * qM[0] + qM[1] * qM[1] - qM[2] * qM[2] - qM[3] * qM[3]);   
	pitch = -asin(2.0f * (qM[1] * qM[3] - qM[0] * qM[2]));
	roll  = atan2(2.0f * (qM[0] * qM[1] + qM[2] * qM[3]), qM[0] * qM[0] - qM[1] * qM[1] - qM[2] * qM[2] + qM[3] * qM[3]);
	pitch *= 180.0f / PI;
	yaw   *= 180.0f / PI; 
    ...
    // the processor speed, and the magnetometer ODR:
...
//-- Function for accelerometer and gyroscope self test; 
//-- check calibration wrt factory settings
void IMU::selfTestMPU9250(float * destination){
	Serial.println("Conducting MPU9250 SeltTest....");
...
void IMU::calibrate(float * dest1, float * dest2){
...
// Fusion of acceleration, rotation rate, and magnetic movements to produce
// a quaternion-based estimate of absolute device orientation which can be 
...
void IMU::MadgwickQuaternionUpdate(float ax, float ay, float az, float gx, float gy, float gz, float mx, float my, float mz)	{
	...
	float q1 = qM[0], q2 = qM[1], q3 = qM[2], q4 = qM[3]; 
	float norm;
